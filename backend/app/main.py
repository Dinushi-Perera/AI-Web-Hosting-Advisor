...
async def lifespan(app:FastAPI):
    ...
    yield
app=FastAPI(title=settings.app_name,version="1.0.0",default_response_class=JSONResponse,lifespan=lifespan,description="Production-oriented FastAPI backend for the AI-Driven Web Hosting Advisor. All monetary API outputs are USD.")
...
async def request_middleware(request:Request,call_next):
    ...
    try:
        if cl and int(cl)>12_000_000:
            return JSONResponse(status_code=413,content={"success":False,"message":"Request body is too large.","code":"REQUEST_TOO_LARGE","error":{"code":"REQUEST_TOO_LARGE"},"requestId":rid})
    except ValueError:
        return JSONResponse(status_code=400,content={"success":False,"message":"Invalid Content-Length header.","code":"BAD_REQUEST","error":{"code":"BAD_REQUEST"},"requestId":rid})
...
async def app_error(request:Request,exc:AppError):return JSONResponse(status_code=exc.status_code,content={"success":False,"message":exc.message,"code":exc.code,"data":None,"error":{"code":exc.code,"details":exc.details},"requestId":getattr(request.state,"request_id",None)})
...
def ready():
    r=readiness();return JSONResponse(status_code=200 if r["ready"] else 503,content=r)
