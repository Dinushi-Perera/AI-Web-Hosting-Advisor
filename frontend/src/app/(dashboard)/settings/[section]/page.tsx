...
function Profile() {
  ...
  const [form,setForm]=useState({fullName:"",email:"",experienceLevel:"Beginner"});
  useEffect(()=>{if(profile.data)setForm({fullName:profile.data.fullName,email:profile.data.email,experienceLevel:profile.data.experienceLevel})},[profile.data]);
  const save=useMutation({mutationFn:()=>userService.update({...form,experienceLevel:form.experienceLevel as "Beginner"|"Intermediate"|"Advanced"}),onSuccess:(data)=>{queryClient.setQueryData(["profile"],data);toast.success("Profile saved to the database")},onError:(e)=>toast.error(message(e))});
